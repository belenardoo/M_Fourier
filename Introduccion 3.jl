...
md"""
!!! danger "Objetivos"
	Bienvenidos a este tercer video de la primera semana del MOOC "Aplicaciones de la Transformada de Fourier". En el video de hoy veremos una introducción a la Transformada de Fourier de forma intuitiva, en la que se introduce análisis en frecuencia para funciones no necesariamente periódicas. En el siguiente video se realizará una formalización matemática de la transformada de Fourier.
"""

# ╔═╡ 3422446c-aab0-4560-9f05-c9f034414a9d
md"""
!!! alert "¿De qué se trata la Transformada de Fourier?"
    La Transformada de Fourier es una herramienta que permite obtener los coeficientes de frecuencia de una función no periódica, considerando una variable continua en frecuencia. A su vez, la transformada inversa permite componer o sintetizar una señal a partir de sus componentes de frecuencia continua.

En los videos anteriores aprendimos que una función periódica se puede descomponer como una suma infinita de sinusoides, lo que permite entender la composición en frecuencia de la señal periódica, observando la amplitud de la sinusoide de cada frecuencia que forma parte de la suma. Sería interesante poder observar también los componentes de frecuencia de una señal que no sea periódica. Para esto, es posible observar a esta como si fuera una señal de *período infinito*.
!!! note "Período infinito"
	Es posible observar una señal no periódica como una función periódica, pero de período infinito. En este caso, tenemos lo siguiente
	...
	Al hacer tender $T \rightarrow \infty$ los espaciamientos en el espectro de frecuencias $1/T$ se hacen cada vez más pequeños.
"""

# ╔═╡ 223c250b-b8da-43e6-a5de-0d6c7a7ee99d
md"""
!!! warning "Ejemplo"
	Veamos ahora un ejemplo que muestra la comparación entre la Serie y la Transformada de Fourier cuando el período de la señal se hacer crecer. En este ejemplo, un pulso cuadrado, a medida que el período tiende a infinito, se convierte en una función rect. Vemos que los coeficientes de la Serie de Fourier coinciden en amplitud con la Transformada de Fourier continua de un rect, que es una función sinc.
"""

# ╔═╡ f8a9479d-f00b-4bb8-b07b-77ecb5f76e72
begin
	gr(size=(700,700))
	function fourierSeries_cn(period, N)
		T = length(period)
		t = collect(0:T-1)
		result = []

		for n in 0:1:N
			c_plusn = sum((1/T) .* (period .* exp.(-2 .* im .* pi .* n .* t ./T)))
			c_minusn = sum((1/T) .* (period .* exp.(2 .* im .* pi .* n .* t ./T)))
			push!(result, [c_plusn, c_minusn])
		end
		return result
	end
	...
	anim = @animate for P = 1:1:40
		
		Fs = 1000
		t = collect(0:1/Fs:50)
		t_period = collect(0:1/Fs:P)
		x_p(t) = abs(mod(t, P) - 0.5) <= 0.5
		x(t) = abs(t - 0.5) <= 0.5
		plot1 = plot(t, x_p.(t), label = false,xlims=(0,40), margin=5mm)
		#println(Fs)
	
		cn = getindex.(fourierSeries_cn(x_p.(t_period), 100), 1)
	    f_discrete = collect(0:length(cn)) ./ P
		
		
		f = collect(LinRange(0, Fs, length(t) + 1))[1:length(t)]
	
		X = fft(x.(t)) ./ Fs
		plot2 = plot(f, real.(X), label = L"\textrm{Transformada \; de \; Fourier \; de \; un \; rect}", xlims = (0, 2.5),margin=5mm)
		plot!( f_discrete, real.(cn .* P), label = L"\textrm{Serie \; de \; Fourier \; de \; un \; rect \; periodico}", line = :stem, linecolor = :red, marker = :circle, margin=5mm)

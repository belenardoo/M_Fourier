...
md"""
!!! danger "Objetivos"
	Bienvenidos a este octavo video de la primera semana del MOOC "Aplicaciones de la Transformada de Fourier". En esta sección aprenderemos sobre la Transformada de Fourier Discreta y un algoritmo para su implementación computacional.
"""

# ╔═╡ bd449efa-c07c-4e27-aff9-a74c1cab944b
md"""
## Definición

La [transformada de Fourier discreta *(Wikipedia)*](https://en.wikipedia.org/wiki/Discrete_Fourier_transform) (DFT) es una transformada para señales discretas y periódicas en el tiempo y en la frecuencia. Esta se define como:

!!! note "DFT"
	$$\tilde{F}[k] = \frac{1}{N} \sum_{n=0}^{N-1} \tilde{f}[n] \; e^{-i k \frac{2 \pi}{N} n}$$
...
!!! note "DFT inversa"
	$$\tilde{f}[n] = \sum_{k=0}^{N-1} \tilde{F}[k] \; e^{i k \frac{2 \pi}{N} n}$$
...
# ╔═╡ d3e8a277-ff1e-4e57-a41d-3ffe58bdec8e
md"""
## Propiedades DFT
...
# ╔═╡ b6f1ab7c-3c60-4f7c-9359-258d97612384
md"""
## FFT
La [transformada discreta de Fourier (DFT)](SS-E-DFT.jl.html) puede implementarse computacionalmente de manera muy eficiente mediante la [transformada rápida de Fourier (FFT) (*Wikipedia*)](https://en.wikipedia.org/wiki/Fast_Fourier_transform).
...
donde $W_N$ denota la base de Fourier $e^{-i \frac{2 \pi}{N}}$. La evaluación de la definición para $k = 0,1,\dots,N-1$ requiere $N^2$ multiplicaciones y $N \cdot (N-1)$ sumas. La complejidad numérica de la DFT es de [orden *(Wikipedia)*](https://en.wikipedia.org/wiki/Big_O_notation) $\mathcal{O} (N^2)$.
...
!!! alert "Separación de la DFT en sus partes par e impar"
	$$\tilde{F}[k] = \frac{1}{N}\left(\underbrace{\sum_{m = 0}^{\frac{N}{2} -1}\tilde{f}[2 m] \, W_N^{k 2m}}_{\tilde{F}_e[k] =\text{DFT de elementos pares}} + \underbrace{\sum_{m = 0}^{\frac{N}{2} - 1} \tilde{f}[2 m + 1] \, W_N^{k (2 m +1)}}_{\tilde{F}_o[k] = \text{DFT de elementos impares}} \right)$$
...
!!! note "Calculo de la frecuencia baja (suma)"
	$$F[0] = \frac{1}{2} (f[0] + W_{2}^{0}f[1]) = \frac{1}{2}(f[0]+f[1])$$

!!! note "Calculo de las frecuencia alta (resta)"
...
# ╔═╡ bef97204-4470-43bf-bec6-9a5e439ac1c6
load("imagenes/FFT1.png")

# ╔═╡ ac3709d9-48a1-452e-a943-e7a919088b78
md"""
!!! warning "La DFT se resume en sumas y restas"

...
md"""
!!! danger "Objetivos"
	Bienvenidos a este segundo video del MOOC "Aplicaciones de la Transformada de Fourier". En el segundo video de esta semana formalizaremos matemáticamente la serie de Fourier, estudiando la forma trigonométrica, la forma compleja y cómo se obtienen los coefientes de Fourier de una funión periódica.
"""

# ╔═╡ 040f0c66-d967-4614-a41b-ba7a3a7aaa94
md"""
## Forma Trigonométrica

Anteriormente describimos la Serie de Fourier como una representación de una función preiódica como suma de sinusoides, lo que significa que una función $f(t)$ periódica de período $T$ puede expresarse como una suma de senos y cosenos de diferentes frecuencias, donde cada sinusoide tiene una aplitud llamada coeficiente. Esto se puede expresar matemáticamente de la siguiente forma:

!!! note "Forma trigonométrica"
	$$f(t) = \frac{a_0}{2} + \sum_{n=1}^{\infty} a_n(f) \cos (2\pi n \frac{t}{T}) + b_n(f) \sin (2\pi n \frac{t}{T})$$

	donde 

	$$a_0 = 2c_0$$

	$$a_n(f)= \frac{2}{T}\int_{0}^{T}f(t)\cos (2\pi n t/T)  dt$$

	$$b_n(f)= \frac{2}{T}\int_{0}^{T}f(t) \sin (2\pi n t/T) dt$$

Esta forma es especialmente útil cuando $f$ es real (dado $a_n$ y $b_n$ son reales) o si $f$ tiene ciertas propiedaes de simetría (entonces se obtiene una serie de senos, que son una función impar, o cosenos, que son una función par). 

Una pregunta muy válida que hacerse luego de observar la expansión en serie de Fourier es cómo se obtienen los coeficientes $a_n$ y $b_n$. Para esto observaremos la forma compleja de la serie, la demostración del cálculo de los coeficientes complejos y usaremos la identidad de Euler.
"""

# ╔═╡ 1fc8ea4c-3366-42dd-abde-71b98aab3067
md"""
## Forma Compleja

Recordemos que la Serie de Fourier es una representación de una función preiódica como suma de sinusoides. En este contexto es muy útil recordar la identidad de Euler:

> $e^{i\theta} = \cos(\theta)+i\sin(\theta)$

A partir de esto, es posible expresar la Serie de Fourier, que es una suma de sinusoides, como una suma de exponenciales complejas.
...
>Dotar al espacio de un producto interno permite definir la *ortonormalidad* de dos elementos del espacio. Dos elementos del espacio son ortonormales si $$( f , g ) = 1$$
...
!!! alert "Teorema de la Series de Fourier"
	$$S_f(t)= \sum_{n \in \mathbb{Z}} c_n(f) e^{i 2 \pi n t/T}$$

	donde los coeficientes $c_n(f)$ están dados por:

	$$c_n(f)=(e^{i 2 \pi n t/T}|f )= \frac{1}{T}\int_{0}^{T} f(t)e^{-i 2 \pi n t/T} dt$$


!!! danger "Descomposición en exponenciales complejas"
    Esta idea es muy poderosa. El hecho que una función periódica pueda descomponerse en una suma de funciones bases exponenciales complejas, cada una de ellas ponderada por el producto interno entre la función y cada una de las mismas bases, implica que las **bases de Fourier son ortonormales**, es decir, el producto interno entre ellas es 1:

	$$\left( e^{i 2 \pi n t/T},e^{-i 2 \pi n t/T}\right) = \frac{1}{T}\int_0^Te^{i 2 \pi n t/T}e^{-i 2 \pi n t/T}dt$$

	$$\left( e^{i 2 \pi n t/T},e^{-i 2 \pi n t/T}\right) =\frac{1}{T}\int_0^T1dt = \frac{T-0}{T} = 1$$

!!! alert "¿Cómo encontrar los coeficientes?"
	...
	Esto se debe a la propiedad de ortogonalidad de las exponenciales complejas y la operación que hemos realizado formalmente se conoce como el producto interno entre dos funciones.
	...
	$$c_k  = \frac{1}{T}\int_{0}^{T} e ^ {- i 2 \pi k t/T} f(t) dt$$
...
### Relación entre la Forma Compleja y la Forma Trigonométrica

La relación explícita viene de la Fórmula de Euler:

> $e^{i\theta} = \cos(\theta) + i\sin(\theta)$

Consideremos la Serie de Fourier compleja:

>$$S_f(t)= \sum_{n \in \mathbb{Z}} c_n(f) e^{i 2 \pi n t/T}$$

Si ahora reemplazamos la exponencial compleja como suma de senos y cosenos:

>$$S_f(t)= \sum_{n \in \mathbb{Z}} c_n(f) \left(\cos\left(2 \pi n t/T\right)+i\sin\left( 2 \pi n t/T\right)\right)$$

Utilizando distributividad de la suma:

>$$S_f(t)= \sum_{n \in \mathbb{Z}} c_n(f) \cos\left(2 \pi n t/T\right)+ c_n(f)i\sin\left( 2 \pi n t/T\right)$$

Considerando que la Forma Compleja es análoga a la Forma Trigonométrica, es necesario establecer la siguiente relación entre los coeficientes $a_n(f)$, $b_n(f)$ y $c_n(f)$:

!!! note "Relación entre Coeficientes"
	$$a_0(f) = 2c_0(f)$$
	$$a_n(f)= c_n(f) + c_{-n}(f) = \frac{2}{T}\int_{0}^{T}f(t)\cos (2\pi n t/T)  dt$$
	$$b_n(f)= i(c_n(f)-c_{-n}(f)) = \frac{2}{T}\int_{0}^{T}f(t) \sin (2\pi n t/T) dt$$
"""
